...
describe('Avatar Component', () => {
  ...
  it('clips a hexagon that is wider than it is tall', () => {
    const wrapper = mount(Avatar, {
      props: { name: 'John Doe', shape: 'hexagon' }
    })
    const style = wrapper.find('.container').attributes('style') ?? ''
    const clip = style.match(/--va-clip-path:\s*(polygon\([^)]*\))/)?.[1] ?? ''
    const points = [...clip.matchAll(/([\d.]+)%\s+([\d.]+)%/g)].map(
    ...
    // The box is square, so an equal span on both axes would stretch the
    // hexagon vertically. A regular one is sqrt(3)/2 as tall as it is wide.
    expect(width).toBeGreaterThan(height)
    expect(height / width).toBeCloseTo(Math.sqrt(3) / 2, 2)
  })
})
