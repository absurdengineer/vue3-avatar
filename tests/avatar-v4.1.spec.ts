...
import { h } from "vue";
import { payloads } from "./helpers/emitted";
...
describe("Avatar v4.1 New Features", () => {
  ...
  describe("Status Indicator Positioning", () => {
    it('has default statusPosition of "bottom-right"', () => {
      ...
      expect(style).toContain("right: 3px");
    });
    ...
    it("does not inset the indicator on square avatars", () => {
      const wrapper = mount(Avatar, {
        props: { name: "John Doe", status: "online", shape: "square" },
      });
    ...
    it("insets further on hexagons, whose corners are furthest from the edge", () => {
      const wrapper = mount(Avatar, {
        props: { name: "John Doe", status: "online", shape: "hexagon" },
      });
      const style = wrapper.find(".status-indicator").attributes("style");
  ...
  describe("AvatarGroup v4.1 Interactivity", () => {
    it("emits two arguments on overflow-click: hidden users and all users", async () => {
      ...
      const args = payloads(wrapper, "overflow-click");
      expect(args).toHaveLength(2);

      const [hidden, all] = args as [{ name: string }[], { name: string }[]];
